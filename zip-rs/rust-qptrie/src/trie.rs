...
impl<TK: PartialEq + AsRef<[u8]>, TV> Trie<TK, TV> {
    ...
    pub fn get<K>(&self, key: &K) -> Option<&TV>
    where
        TK: Borrow<K>,
        K: AsRef<[u8]> + ?Sized,
    {
        ...
        K: AsRef<[u8]> + ?Sized,
    {
        ...
        while let Some(t) = todo.pop() {
            match *t {
                Node::Leaf(ref leaf) => {
                    if leaf.key.borrow().as_ref().starts_with(prefix.as_ref()) {
    ...
    pub fn prefix_iter<'t, K: ?Sized>(&'t self, prefix: &'t K) -> TriePrefixIterator<K, TK, TV>
    where
        TK: Borrow<K>,
        K: AsRef<[u8]>,
    {
