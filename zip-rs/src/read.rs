...
use types::{DateTime, System, ZipFileData};
...
/// A string that is always ascii lowercase
#[repr(transparent)]
#[derive(PartialEq)]
pub struct AsciiLowercaseString(String);
/// A str that is always ascii lowercase
#[repr(transparent)]
#[derive(PartialEq)]
pub struct AsciiLowercaseStr(str);

impl From<String> for AsciiLowercaseString {
    fn from(mut other: String) -> Self {
        other.make_ascii_lowercase();
        AsciiLowercaseString(other)
    }
}

impl Borrow<AsciiLowercaseStr> for AsciiLowercaseString {
    fn borrow(&self) -> &AsciiLowercaseStr {
        use std::mem;

        let string: &str = &self.0;
        unsafe { mem::transmute(string) }
    }
}

impl AsRef<AsciiLowercaseStr> for AsciiLowercaseString {
    fn as_ref(&self) -> &AsciiLowercaseStr {
        self.borrow()
    }
}

impl<T: ?Sized> AsRef<T> for AsciiLowercaseString
where
    String: AsRef<T>,
{
    fn as_ref(&self) -> &T {
...
impl<T: ?Sized> AsRef<T> for AsciiLowercaseStr
where
    str: AsRef<T>,
{
    fn as_ref(&self) -> &T {
...
/// ```
#[derive(Clone, Debug)]
pub struct ZipArchive<R, S = String> {
    names_map: Trie<S, usize>,
    reader: R,
...
impl<R: Read + io::Seek, S: PartialEq + AsRef<[u8]> + From<String>> ZipArchive<R, S> {
    ...
    pub fn new(mut reader: R) -> ZipResult<Self> {
        ...
        for _ in 0..number_of_files {
            ...
            let name = file.file_name.clone().into();
            names_map.insert(name, files.len());
            files.push(file);
        }
    ...
    /// Gets all files that start with a given prefix (for example, to read a directory)
    pub fn starting_with<'a, Q: ?Sized>(
        &'a self,
        name: &'a Q,
    ) -> impl Iterator<Item = (&Q, usize)> + 'a
    where
        Q: AsRef<[u8]>,
        S: Borrow<Q>,
    {
        // TODO: Use `UniCase` fork (need to fork since `UniCase` doesn't impl `AsRef<[u8]>`
        self.names_map
            .prefix_iter(name)
            .map(|(k, v)| (k.borrow(), *v))
    }
    ...
    pub fn by_name<'a, Q: ?Sized>(&'a mut self, name: &Q) -> ZipResult<ZipFile<'a>>
    where
        Q: AsRef<[u8]>,
        S: Borrow<Q>,
    {
